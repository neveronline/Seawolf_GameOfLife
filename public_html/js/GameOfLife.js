 ...
 * 
 * Authors: Richard McKenna & _______________
 */
...
function initConstants()
{
    ...
    TEXT_COLOR = "#7777CC";
    INIT_COLOR ="rgba(102, 204, 255,0.5)";
    VOID_COLOR = "#BFBFBF";
...
function respondToMouseClick(event)
{
    ...
    if(patternsList.selectedIndex < 24){
    ...
    for (var i = 0; i < pixels.length; i += 2)
        {
            ...
            setGridCell(colorArray,row,col,GHOST_CELL);
        }
    }
...
function respondToMouseMove(event)
{
    ...
    // if we already have a save coordinate, then we want to delete last pattern
    
        for (var i = 0; i < pixels.length; i += 2)
        {
            var col = clickCol + pixels[i];
            var row = clickRow + pixels[i+1];
            setGridCell(ghostShapeArray, row, col, GHOST_CELL);
        }
    // the mouse is press down and select void cell
    if(patternsList.selectedIndex == 24 && click == 1)
    {
    ...
    // the mouse is press down and select void cell
    if(patternsList.selectedIndex == 25 && click == 1)
    {
        ...
        for (var i = 0; i < pixels.length; i += 2)
        {
            var col = clickCol + pixels[i];
            var row = clickRow + pixels[i+1];
            var tempcell = getGridCell(renderGrid,row,col);
            
            if(tempcell == VOID_CELL)
            {
            var x = col * cellLength;
            var y = row * cellLength;
...
function ghotstCellRender()
{
    ...
    canvas2D.fillStyle = INIT_COLOR;
    for (var i = 0; i <= gridHeight; i++)
    {
        for (var j = 0; j < gridWidth; j++)
            {
                var cell = getGridCell(ghostShapeArray, i, j);
                if (cell === GHOST_CELL)
                    {
                        var x = j * cellLength;
                        var y = i * cellLength;
        ...
        for (var i = 0; i <= gridHeight; i++)
        {
           for (var j = 0; j < gridWidth; j++)
               {
                   var cell = getGridCell(colorArray, i, j);
                   if (cell === GHOST_CELL)
                       {
                           var x = j * cellLength;
                           var y = i * cellLength;
